...
public class AlumnoService {
	...
	@Transactional
	public Alumno inscribirAlumnoACursos(InscripcionDTO dto) {
		Alumno alumno = alumnoRepository.findById(dto.getAlumnoId())//recuperado el alumno ID
				.orElseThrow(()-> new IllegalArgumentException("Alumno No encontrado"));
		for (Long cursoId : dto.getCursoIds()) {
			Curso curso = cursoRepository.findById(cursoId)
					.orElseThrow (()-> new IllegalArgumentException("Curso no encontrado"));
			alumno.getCursos().add(curso);//JOIN ES LO QUE HACE
